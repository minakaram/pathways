...
cvNameInput.addEventListener('click', () => {
    cvInput.click();
});
...
cvInput.addEventListener('change', () => {
    if (cvInput.files.length > 0) {
        cvNameInput.value = cvInput.files[0].name;
    }
});
...
// Open file dialog when clicking the text input
coverLetterNameInput.addEventListener('click', () => {
...
coverLetterInput.addEventListener('change', () => {
    if (coverLetterInput.files.length > 0) {
        coverLetterNameInput.value = coverLetterInput.files[0].name;
    }
});


$(document).ready(function () {
    $(".desc-box-heading").click(function () {
        // Slide toggle the content inside the clicked box
        $(this).next(".desc-box-content").slideToggle();

        // Rotate the arrow 180 degrees by toggling a class
